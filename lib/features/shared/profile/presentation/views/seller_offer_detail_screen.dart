...
class SellerOfferDetailScreen extends ConsumerWidget {
  ...
  Widget _buildImage(String url, {BoxFit fit = BoxFit.cover, int? cacheSize}) {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    for (final raw in offer.imageUrls) {
      ...
      if (hasCover && normalizeImageKey(url) == normalizeImageKey(coverUrl))
        continue;
    ...
    final coverImageCacheSize = (MediaQuery.sizeOf(context).width * dpr)
        .round();
            ...
            flexibleSpace: FlexibleSpaceBar(
              background: Stack(
                ...
                children: [
                  // Cover image
                  hasCover
                      ? _buildImage(coverUrl, cacheSize: coverImageCacheSize)
                      : Container(
                          ...
                          GestureDetector(
                            ...
                            child: Container(
                              ...
                              decoration: BoxDecoration(
                                color: categoryBgRed,
                                borderRadius: BorderRadius.circular(16),
                                border: Border.all(color: categoryTextRed),
                              ),
                              child: const Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
    ...
    try {
      ...
      if (context.mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Error al pausar donación: $e')));
      }
    }
  }
  ...
  void _deleteOffer(BuildContext screenContext, WidgetRef ref) {
    showDialog(
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              try {
                ...
                ScaffoldMessenger.of(screenContext).showSnackBar(
                  const SnackBar(content: Text('Donacion eliminada')),
                );
              } catch (e) {
                if (!screenContext.mounted) return;
                ScaffoldMessenger.of(screenContext).showSnackBar(
...
class _UnansweredQuestionsBadge extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final unansweredCount = ref
        .watch(offerCommentsProvider(offerId))
        .maybeWhen(
          data: (comments) =>
