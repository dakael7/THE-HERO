...
class InPersonBuyerRatingScreen extends ConsumerStatefulWidget {
  final Order order;
  final String sellerId;

  const InPersonBuyerRatingScreen({
    super.key,
    required this.order,
  ...
  @override
  ConsumerState<InPersonBuyerRatingScreen> createState() =>
      _InPersonBuyerRatingScreenState();
}

class _InPersonBuyerRatingScreenState
    extends ConsumerState<InPersonBuyerRatingScreen> {
  int _rating = 5;
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(orderActionsProvider).rateBuyerForInPersonPickup(
            orderId: widget.order.orderId,
            sellerId: widget.sellerId,
            buyerRating: _rating.toDouble(),
            buyerComment: _commentController.text.trim().isEmpty
                ? null
  ...
  Widget build(BuildContext context) {
    final buyerAsync = ref.watch(userByIdStreamProvider(widget.order.heroId));
      ...
      body: ListView(
        ...
        children: [
          _SuccessCard(
            ...
            message: 'Califica tu experiencia con el Hero que hizo el pedido.',
          ),
          const SizedBox(height: 16),
          _RatingCard(
            roleLabel: 'Hero comprador',
            name: buyerName,
